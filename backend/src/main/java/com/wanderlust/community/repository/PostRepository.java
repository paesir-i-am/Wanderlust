...
@Query("SELECT p FROM Post p WHERE p.authorNickname = :authorNickname AND p.isDeleted = false ORDER BY p.id DESC ")
  Page<Post> findAllNotDeletedByAuthorNickname(@Param("authorNickname") String authorNickname, Pageable pageable);
}
