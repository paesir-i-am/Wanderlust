...
const ParkingInfoPage = () => {
  ...
  const [selectedTerminal, setSelectedTerminal] = useState(queryParams.get('terminal') || 'T1'); // 기본값 'T1'
  const [selectedParkingType, setSelectedParkingType] = useState(queryParams.get('parkingType') || 'short'); // 기본값 'short'
  ...
  const handleSearchClick = () => {
    ...
    if (selectedTerminal && selectedParkingType) {
      navigate(`/flightApi/parking-info?terminal=${selectedTerminal}&parkingType=${selectedParkingType}`);
    }
  };
    ...
    <div className="parking-info-page">
      ...
      <div className="selected-info">
        ...
        <div className="selected-info-text">
          <p><strong>선택된 터미널:</strong> {selectedTerminal === 'T1' ? '제 1 여객터미널' : '제 2 여객터미널'}</p>
          <p><strong>선택된 주차장 유형:</strong> {selectedParkingType === 'short' ? '단기주차장' : '장기주차장'}</p>
        </div>
