...
import BasicLayoutWithoutFlight from "../../../common/layout/basicLayout/BasicLayoutWithoutFlight"; // 상대경로 확인
    ...
    const filterParkingData = () => {
      ...
      if (selectedTerminal) {
        filteredData = filteredData.filter(
          (item) => item.floor && item.floor.includes(selectedTerminal),
        );
      }
      ...
      if (selectedParkingType === "short") {
        filteredData = filteredData.filter(
          (item) => item.floor && item.floor.toLowerCase().includes("단기"),
        );
      } else if (selectedParkingType === "long") {
        // '장기'가 포함된 항목만 필터링
        filteredData = filteredData.filter(
          (item) => item.floor && item.floor.toLowerCase().includes("장기"),
        );
      }
            ...
            filteredParkingData.map((item, index) => (
              <div key={index} className="parking-level">
                ...
                <div className="parking-info">
                  <p>
                    <strong>총 주차면적:</strong> {item.parkingarea || "0"}
                  </p>
                  <p>
                    <strong>주차된 차량 수:</strong> {item.parking || "0"}
                  </p>
                  <p>
                    <strong>남은 주차 공간:</strong>{" "}
                    {calculateAvailableSpaces(item.parking, item.parkingarea)}
                  </p>
                  <p>
                    <strong>실시간 주차 현황:</strong>{" "}
                    {formatDate(item.datetm) || "알 수 없음"}
                  </p>
                </div>
              </div>
            ))
